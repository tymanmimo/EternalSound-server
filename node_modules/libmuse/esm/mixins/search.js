...
import { BADGE_LABEL, MRLIR, MRLITFC, MUSIC_SHELF, NAVIGATION_BROWSE_ID, NAVIGATION_VIDEO_ID, RUN_TEXT, SECTION_LIST, TEXT_RUN_TEXT, THUMBNAILS, TITLE_TEXT, } from "../nav.js";
import { _, __ } from "../parsers/browsing.js";
import { filters, get_search_params, parse_search_results, parse_top_result, scopes, } from "../parsers/search.js";
...
export async function get_search_suggestions(query, options = {}) {
        ...
        for (const item of items) {
            if ("historySuggestionRenderer" in item) {
                ...
                suggestions.history.push({
                    ...
                    feedback_token: j(query, "serviceEndpoint.feedbackEndpoint.feedbackToken"),
...
export async function remove_search_history(token, options = {}) {
    await check_auth();
...
export async function search(query, options = {}) {
    ...
    if (!continuation) {
        ...
        const chips = jo(results, "sectionListRenderer.header.chipCloudRenderer.chips");
        if (chips) {
            chips
        ...
        for (const res of section_list) {
            if ("musicShelfRenderer" in res) {
                ...
                const category_search_results = parse_search_results(results, scope ?? null, filter ?? null);
                if (category_search_results.length > 0) {
                    search_results.categories.push({
                        title: category ?? null,
                        filter: __(category) ?? null,
                ...
                search_results.top_result = parse_top_result(j(res, "musicCardShelfRenderer"));
            }
            else if ("itemSectionRenderer" in res) {
                const did_you_mean = jo(res, "itemSectionRenderer.contents[0].didYouMeanRenderer");
                if (did_you_mean) {
                    search_results.did_you_mean = {
                        search: j(did_you_mean, "correctedQuery.runs"),
                        query: j(did_you_mean, "correctedQueryEndpoint.searchEndpoint.query"),
                    };
                }
                const autocorrect = jo(res, "itemSectionRenderer.contents[0].showingResultsForRenderer");
                if (autocorrect) {
                    search_results.autocorrect = {
                        corrected: {
                            search: j(autocorrect, "correctedQuery.runs"),
                            query: j(autocorrect, "correctedQueryEndpoint.searchEndpoint.query"),
                        },
                        original: {
                            search: j(autocorrect, "originalQuery.runs"),
                            query: j(autocorrect, "originalQueryEndpoint.searchEndpoint.query"),
                        },
                    };
                }
            }
...
export async function get_more_search_results(continuation, options) {
    const { limit = 20, scope, filter, signal } = options;
    const continued_data = await get_continuations(continuation, "musicShelfContinuation", limit, (params) => {
