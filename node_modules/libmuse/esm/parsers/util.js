import { find_objects_by_key, MENU_ITEMS, NAVIGATION_BROWSE_ID, } from "../nav.js";
import { j, jo } from "../util.js";
export function get_menu_playlists(data) {
    const ids = {
        shuffleId: null,
        radioId: null,
    };
    const watch_menu = find_objects_by_key(j(data, MENU_ITEMS), "menuNavigationItemRenderer");
    for (const item of watch_menu.map((menu) => menu.menuNavigationItemRenderer)) {
        const icon = j(item, "icon.iconType");
        const watch_id = jo(item, "navigationEndpoint.watchPlaylistEndpoint.playlistId") ??
...
export function get_item_text(item, index, run_index = 0) {
...
export function get_flex_column_item(item, index) {
...
export function get_fixed_column_item(item, index) {
    if (!("text" in
...
export function get_browse_id(item, index) {
...
export function get_dot_separator_index(runs) {
    const index = runs.findIndex((run) => run.text === " • ");
    return index < 0 ? runs.length : index;
}
export function color_to_hex(a) {
    const arr = [
        (a & 16711680) >>> 16,
        (a & 65280) >>> 8,
        a & 255,
